...
import com.gerasimovd.rickmorty.adapters.location.LocationsAdapter
...
import com.gerasimovd.rickmorty.viewmodel.location.LocationsViewModel
...
class LocationsFragment : Fragment(), ItemClickListener {
    ...
    override fun <T> onClick(item: T) {
        item as Location
        (requireActivity() as MainActivity).navigateTo(LocationInfoFragment.newInstance(item.id))
    }
    ...
    fun scrollToTop() {
        if (recyclerAdapter.itemCount != 0)
            binding.locationsRecycler.smoothScrollToPosition(0)
    }
    ...
    private fun setupAdapter(): LocationsAdapter {
        recyclerAdapter = LocationsAdapter(this)
    ...
    private fun fetchData(searchInputText: String = "") {
        lifecycleScope.launch {
            viewModel.getLocations(searchInputText).collectLatest {
